...
var Business = {
  ...
  renderReferenceComponent: async function(component = {}) {
    ...
    // 获取具体组件
    var instanceComponent = className.indexOf("el-") > -1 ? ElementPlus[componentName] : vant[componentName]
    var refObj = {}
    component.properties.forEach(item => {
      refObj[item.attr] = item.value
    })
    // 创建响应式对象
    var store = Vue.ref(refObj)
    // 创建虚拟reference插槽按钮  通过父元素事件传递触发
    var VNode = Vue.createVNode(instanceComponent, {
      ...store.value,
    }, {
      reference: () => Vue.createVNode('button', { 
        style: {
          opacity: 0,
          width: "1px",
          height: "1px",
    ...
    Vue.render(VNode, bindElement)
    bindElement.addEventListener("click", function() {
      bindElement.querySelector(".referenceBtnEvent").click()
    })   
    // _Global.configDomList.push({
    //   component,
